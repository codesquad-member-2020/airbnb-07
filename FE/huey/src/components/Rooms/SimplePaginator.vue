<template>
  <nav aria-label="pagination flex">
    <ul v-if="showPaginator" class="pagination flex list-reset">
      <li
      ...
      <li class="page-item m-1" v-for="(page, index) in pages" :key="index">
        <a
          class="text-xs no-underline page-link p-2 w-8 h-8 flex inline-block justify-center items-center"
          :class="
  ...
  computed: {
    ...
    pages() {
      ...
      let pagesArr = Array.from(
        Array(pages),
        (val, i) => (this.currentSection - 1) * this.pagesPerSection + i + 1,
      );
      ...
      if (pagesArr.length < this.pagesPerSection) {
        pagesArr = Array.from(
          ...
          (val, i) =>
            pagesArr[pagesArr.length - 1] - this.pagesPerSection + i + 1,
        );
      }
      return pagesArr;
    },
