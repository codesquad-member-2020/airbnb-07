...
export default {
  ...
  methods: {
    ...
    trapKeyboardInput(event) {
      ...
      const shortcutKeyCodes = Object.keys(this.keys).map(
        key => this.keys[key],
      );
    ...
    handleKeyboardInput(event) {
      if (this.shouldHandleInput(event, this.keys.esc)) {
        ...
        if (
          !this.showKeyboardShortcutsMenu &&
          target &&
          target.tagName === 'TD'
        ) {
    ...
    setupDatepicker() {
      ...
      if (this.$options.colors) {
        ...
        this.colors.hoveredInRange =
          colors.hoveredInRange || this.colors.hoveredInRange;
        this.colors.selectedText =
    ...
    resetFocusedDate(setToFirst) {
      if (this.focusedDate && !this.isDateVisible(this.focusedDate)) {
        ...
        const newFocusedDate = setYear(
          setMonth(this.focusedDate, monthIdx),
          year,
        );
    ...
    isDateVisible(date) {
      ...
      const end = addDays(
        lastDayOfMonth(this.visibleMonths[this.monthsToShow - 1]),
        1,
      );
